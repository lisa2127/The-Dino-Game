...
void init() {
    ...
    for (int i = 0; i < 79; i++){
    	gotoxy(1+i, 1); cout<<"ß"; //Border up
    	gotoxy(1+i, 25); cout<<"ß"; //Border down
	} 
}
...
void play(){ 
	...
	init();
    while (true) {
        while (!kbhit()) {          //keyboard hit present in conio header which returns a
                                        //non zero value when key is pressed
			if( gameover==1 ){
            	return;
			}
			moveDino();
